...
## 6. עקרונות-קבע חוצי-GEM — חובה כשההוראות האמיתיות ייכתבו מחר (RULE 1 / RULE 2, WORK-ID `TRADINGBRAIN-CROSS-GEM-DEDICATED-CONTENT-RULE`, 2026-09-15)

**סעיף זה נוסף היום (2026-09-15) לפי החלטת-משתמש מפורשת, לפני שההוראות עצמן נכתבות.** שני עקרונות-קבע, לא ספציפיים לפונדמנטלי — חלים על **כל** GEM עתידי בנושא שוק-ההון (טכני, מאקרו, אלגו), כפי שכבר יושמו בפועל בין הג'ם הפונדמנטלי (`GEM-FUNDAMENTAL-INSTRUCTIONS.md`) לבין ה-GEM של מבזק בוקר/ערב (`GEM-BRIEF-INSTRUCTIONS.md`).

### RULE 1 — תוכן ייעודי חוצה-נושא (טאב 7)

חילוץ `stockFundamentals`/`stockTechnicals` (ר' `docs/gems/GEM-FUNDAMENTAL-INSTRUCTIONS.md` שורות 81-133 לצורה המלאה) **אסור שיהיה מותנה בנושא המרכזי של הסרטון**. כשה-GEM הטכני ייכתב מחר: אם וידאו שעיקרו ניתוח פונדמנטלי (או כל נושא אחר) מזכיר בדרך-אגב רמה טכנית אמיתית עם ערך וטיקר (תמיכה/התנגדות/ממוצע נע וכו', עם `sourceQuote` ניתן-לאימות) — עדיין יש לחלץ אותה כשורת `stockTechnicals`, לא לדלג עליה מכיוון שהיא לא "נושא הסרטון". הגייט היחיד הוא **עדות** (מדד/רמה + ערך + טיקר אמיתיים שנאמרו בפועל), לא התאמת-נושא.

**⚠️ אימות-מצב חשוב (2026-09-15):** המשימה שהכניסה את העיקרון הזה ציטטה "Outcome 3" קיים כביכול ב-`GEM-FUNDAMENTAL-INSTRUCTIONS.md`, כמנגנון "סירוב סוג-תוכן" שדרכו יש להבטיח שחילוץ טאב-7 שורד. **גרפ+קריאה מלאה של `GEM-FUNDAMENTAL-INSTRUCTIONS.md`, `GEM-FUNDAMENTAL-SCHEMA.md`, `FUNDAMENTAL-OVERVIEW.md` ו-`GEM-BRIEF-INSTRUCTIONS.md` היום לא מצאו שום מנגנון כזה בשום מסמך בריפו** — אין "Outcome"/תרחיש-מספור, ואין נתיב-סירוב מתועד לאף GEM קיים כרגע. **פתוח, ממתין להחלטת-משתמש** (ר' `docs/open-items-ledger.md`). המשמעות המעשית לכתיבה-עתידית: **אם וכאשר** ייכתב מנגנון "סוג-תוכן שגוי" לג'ם הטכני (או לכל GEM אחר) — חובה שהוא **לא** יבטל את חילוץ טאב-7: המנגנון חייב עדיין לסרוק אחר שורות `stockFundamentals`/`stockTechnicals` ולהחזיר אותן (בתוך JSON תקין) גם כשהוא מסרב למלא שאר השדות, לא JSON ריק/מסורב-לגמרי.

### RULE 2 — שמות-שדה אחידים למושגים משותפים (לא key-renaming)

לכל GEM עתידי בנושא שוק-ההון: כשתוכן ממלא **תפקיד משותף** עם מה שכבר קיים בפונדמנטלי/מבזק — יש לעשות בו שימוש חוזר **מילולי-מדויק** בשמות-השדות הבאים, לא לבחור שם חדש/דומה:

| מושג משותף | שם-שדה מחייב | מקור-האמת |
|---|---|---|
| נקודות-נתון פונדמנטליות לפי טיקר | `stockFundamentals` | `GEM-FUNDAMENTAL-INSTRUCTIONS.md` שורות 81-133 |
| נקודות-נתון טכניות לפי טיקר | `stockTechnicals` | אותה צורה בדיוק, `metric`→`levelType` |
| דגל נתון בר-רענון | `liveDataAvailable`/`liveDataNote` | `GEM-FUNDAMENTAL-INSTRUCTIONS.md` שורות 110-117 |
| כלל-מסחר חוזר, מובנה-predicate | `methodologicalRules` (וצורת-התנאי `{indicator,comparator,value,unit,timeframe,assetType,rawPhrase,thresholdDefined}`) | `GEM-FUNDAMENTAL-INSTRUCTIONS.md` שורות 222-247 |
| 7 טאבי-UI קבועים | `UNIVERSAL_TABS` (מנגנון-אפליקציה, לא שדה ב-JSON) | `VideoDetailPanel.jsx:3042,10341` |

**חשוב — זה לא task של שינוי-שמות בקוד קיים.** `indicators`/`setups`/`patterns` (שדות-התוכן הדומיננטיים-קיימים של הנתיב הטכני, ר' סעיף 3 למעלה) **נשארים כפי שהם** — הם תוכן-דומיין ספציפי-לטכני, לא "מושג משותף" עם הפונדמנטלי, ואין להם מקבילה-שם לאחד. RULE 2 חל **רק** על ארבעת המושגים בטבלה למעלה + מבנה-7-הטאבים — לא על כל שדה שקיים בקוד. פירוט מלא + טבלת-התאמה בין `universalTabs` (מבזק) לשדות-שורש (פונדמנטלי): `docs/gems/GEM-SHARED-CONVENTIONS.md` (חדש).

### אין הקלה על כללי אנטי-המצאה

שני העקרונות למעלה **אינם** מקלים על שום כלל אנטי-המצאה/zero-inference שכבר קיים בהוראות הפונדמנטליות/מבזק (שורות 30/265-271 ב-`GEM-FUNDAMENTAL-INSTRUCTIONS.md`, "EVIDENCE RULES" ב-`GEM-BRIEF-INSTRUCTIONS.md`) — "חוצה-נושא" משמעו רק שההיקף-הנושאי לא חוסם חילוץ, **לא** שרף-הראיה יורד. שורת `stockTechnicals`/`stockFundamentals`/`methodologicalRules` עדיין נכתבת **רק** כשהמספר/הרמה/התנאי נאמרו בפועל, עם `sourceQuote` אמיתי — בדיוק כמו כל שדה אחר.
