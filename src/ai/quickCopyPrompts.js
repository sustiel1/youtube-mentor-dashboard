...
export function buildGeminiNewsQuickPrompt({ title, duration, transcript }) {
  ...
  return [
    ...
    '• marketNews: כל פריט הוא object עם title, summary, impact, sentiment, sentimentReason, sentimentConfidence ו-sourceEvidence. sentiment חייב להיות positive / negative / neutral / mixed / unknown.',
    '• סווג sentiment ביחס להיקף מפורש ורק לפי ראיות: בפועל מול תחזית, שינוי תחזית, צמיחה/ירידה, מגבלה רגולטורית, היצע/ביקוש או תגובת שוק מפורשת. אין להסיק מ-importance או category; כשאין די ראיות השתמש unknown, ובעדויות סותרות mixed.',
    ...
    '• tradingOpportunities: עד 3 הזדמנויות מסחר שונות ומבוססות ראיות, רק כאשר הן נתמכות בתמלול — בעברית',
    '• risks: עד 3 סיכונים שונים ומבוססי ראיות, רק כאשר הם נתמכים בתמלול — בעברית',
    '• אם שדה ריק — החזר מערך ריק, לא placeholder',
    '• אסור ערכים גנריים',
    '',
    'כללי JSON קפדניים — חובה:',
    '• החזר JSON טהור בלבד. אסור: ```json, markdown, טקסט לפני/אחרי ה-JSON.',
    '• אסור מרכאות כפולות (") בכל מקום בתוך ערכי מחרוזת — כולל מונחי שוק: כתוב נאסדק (לא נאסד"ק), אגח (לא אג"ח), תא (לא ת"א), חכ (לא ח"כ), זל, עוד, מג, בגץ — ללא גרשיים.',
    '• אסור לחתוך את ה-JSON באמצע — סיים את כל האובייקטים וה-arrays, כולל הסגריות הסוגרות האחרונות }}',
    '• אסור להכניס מבנה JSON גולמי בתוך ערך מחרוזת.',
    '• שורות חדשות בתוך מחרוזות — כתוב \\n (backslash + n), לא שורה חדשה אמיתית.',
    '',
    'JSON schema:',
    schema,
    '',
