...
const _SECTOR_ENTRIES = [
  ...
  ['Software Services',                       { etf: 'IGV',  he: 'תוכנה' }],
  ['Defensive / Software Services',           { etf: 'IGV',  he: 'תוכנה' }],
...
export const HE_CARD_COMPANY_ALIASES = [
  // compound names first — must precede their own prefixes
  ['סופר מיקרו קומפיוטר', 'SMCI'],
  ['סופר מיקרו',          'SMCI'],
  ["לאם ריסרץ׳",          'LRCX'],
  ["לאם ריסרץ'",          'LRCX'],
...
export function enrichWatchTodayItem(rawText) {
  ...
  // Pure uppercase ticker with no description (e.g. "HNET", "NVDA")
  if (/^[A-Z]{2,6}$/.test(text)) {
    return { displayText: text, ticker: text, finvizUrl: buildFinvizQuoteUrl(text) };
  }
