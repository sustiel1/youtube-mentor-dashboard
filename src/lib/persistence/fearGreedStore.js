...
export const FEAR_GREED_CACHE_STORAGE_KEY = 'market_fear_greed_cache_v1';
...
export async function readFearGreedCache() {
  const facade = await getFacade();
  const value = await facade.getJson(FEAR_GREED_CACHE_STORAGE_KEY, null);
  if (!value || typeof value !== 'object' || typeof value.score !== 'number') return null;
  return value;
}

export async function writeFearGreedCache(record) {
  const facade = await getFacade();
  const result = await facade.setRaw(FEAR_GREED_CACHE_STORAGE_KEY, JSON.stringify(record));
  if (!result.ok) {
    throw new Error(`Failed to persist Fear & Greed cache (${result.code})`);
  }
  return record;
}
