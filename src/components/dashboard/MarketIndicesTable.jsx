...
function dirInfo(dir) {
  ...
  if (d === 'up' || d === 'bullish' || d === 'positive')
    return { emoji: '🟢', label: 'עולה', cls: 'text-emerald-600 dark:text-emerald-400' };
  if (d === 'down' || d === 'bearish' || d === 'negative')
    return { emoji: '🔴', label: 'יורד', cls: 'text-red-500 dark:text-red-400' };
  if (d === 'flat' || d === 'neutral' || d === 'unchanged')
    return { emoji: '⚪', label: 'יציב', cls: 'text-slate-400 dark:text-zinc-500' };
  return { emoji: null, label: dir || '—', cls: 'text-slate-500 dark:text-zinc-400' };
}
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="border-b border-slate-200 dark:border-zinc-700 bg-slate-50/80 dark:bg-zinc-900/50">
            ...
            <th className="px-3 py-2 text-right text-xs font-semibold text-slate-500 dark:text-zinc-400">כיוון</th>
        ...
        <tbody>
          {rows.map((row, i) => {
            ...
            return (
              <tr
                key={i}
                className="border-b border-slate-100 dark:border-zinc-800/60 hover:bg-white/70 dark:hover:bg-zinc-800/40 transition-colors group"
              >
                ...
                <td className={`px-3 py-2.5 font-mono text-xs font-semibold whitespace-nowrap ${cc}`}>
                  {row.change || '—'}
                </td>
