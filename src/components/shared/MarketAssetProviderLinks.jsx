...
export function MarketAssetPreferredLink({ asset, className = '', children }) {
  const resolution = resolveMarketAssetProviderLinks(asset);
  const display = children ?? asset ?? '—';
  if (!resolution?.preferred) return <span className={className}>{display}</span>;
...
export function MarketAssetProviderLinks({ asset }) {
  const resolution = resolveMarketAssetProviderLinks(asset);
