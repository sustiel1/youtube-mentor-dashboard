...
- **ALFRED vintage/release-date data**: the true point-in-time REST API (`api.stlouisfed.org/fred/...`) requires an API key (`FRED_API_KEY` env var) and returns a clean 400 without one — confirming the host itself is reachable. Without a key, the closest no-key substitute is scraping the public ALFRED release-calendar HTML page (`alfred.stlouisfed.org/release?rid=<RELEASE_ID>`). **Verify the release ID by checking the page's `<h1>` before trusting it** — release IDs are not obvious from the series ID (e.g. rid 50 = Employment Situation, rid 180 = Unemployment Insurance Weekly Claims Report; a guessed ID can silently land on the wrong report, e.g. rid 97 = New Residential Sales, not Michigan Sentiment). The releases index (`alfred.stlouisfed.org/releases`) is paginated — a single-page grep for a release name proves nothing by itself.
- **FOMC meeting dates**: `federalreserve.gov/monetarypolicy/fomccalendars.htm` is reachable and contains real meeting-date markup; no API, HTML scrape only — re-verify the scrape after any Fed site redesign.
